...
Shader "Shader Forge/OldSchoolPlus_Forge" {
    Properties {
        ...
        _lightPower ("lightPower", Range(0, 1)) = 0.7816603
...
////// Emissive:
                ...
                float node_2780 = saturate(dot(i.normalDir,lightDirection));
                float _power_var = UNITY_ACCESS_INSTANCED_PROP( Props, _power );
                float3 node_9523 = ((_lightColor_var.rgb*_lightPower_var)*((_baseColor_var.rgb*node_2780)+pow(saturate(dot(lightDirection,viewReflectDirection)),_power_var)));
                ...
                float3 emissive = ((attenuation*node_9523)+(((_baseColor_var.rgb*(((_upColor_var.rgb*node_7275)+(node_2704*_downColor_var.rgb))+(_MiddleColor_var.rgb*((node_9109-node_7275)-node_2704))))*_EnvColorLevel_var)*_occoluision_var.rgb));
